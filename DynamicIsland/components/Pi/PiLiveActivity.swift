...
/// Closed-notch live activity shown while the pi CLI is executing a task.
...
struct PiLiveActivity: View {
    ...
    private var phase: PiActivityPhase { piMonitor.phase }
    ...
    private var hasHitRate: Bool { piMonitor.detail?.cacheHitRate != nil }
    ...
    private var hasModel: Bool { piMonitor.model != nil }

    private var thinkingTextWidth: CGFloat {
        guard let level = piMonitor.thinkingLevel else { return 0 }
    ...
    private var measuredElapsedWidth: CGFloat {
        ...
        case .running(let started):
            text = PiLiveActivity.formatElapsed(Date().timeIntervalSince(started))
        case .completed(let at, let startedAt):
            text = startedAt.map { PiLiveActivity.formatElapsed(at.timeIntervalSince($0)) } ?? ""
        case .idle:
            text = ""
        }
        ...
        Rectangle()
            ...
            .overlay(alignment: .top) {
                VStack(spacing: 0) {
                    ...
                    if showsDetail, let detail = piMonitor.detail {
                        detailPanel(detail)
    ...
    private var topRowContent: some View {
        HStack(spacing: 0) {
                    ...
                    if let level = piMonitor.thinkingLevel {
                        Text(level)
                            .font(thinkingFont)
                    ...
                    if let hit = piMonitor.detail?.cacheHitRate {
                    ...
                    case .completed(let at, let startedAt):
                        ...
                        if let error = piMonitor.detail?.errorMessage {
...
/// Compact right-wing supplement shown beside the music live activity while pi
/// is executing a task (mirrors `MusicTimerSupplementView`). On completion the
/// elapsed counter swaps for a stroke-drawn checkmark; the music view's own
/// `secondary?.id` animation handles the swap and the final dismissal.
struct PiMusicSupplementView: View {
    @ObservedObject var monitor: PiSessionMonitor
    let notchHeight: CGFloat

    @State private var spinning = false
    @State private var checkProgress: CGFloat = 0

    private var phase: PiActivityPhase { monitor.phase }
    private var isCompleted: Bool {
        if case .completed = phase { return true }
        return false
    }
...
/// The ✓ stroke, drawn progressively via `trim`.
struct CheckmarkShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        path.move(to: CGPoint(x: rect.width * 0.16, y: rect.height * 0.54))
        path.addLine(to: CGPoint(x: rect.width * 0.40, y: rect.height * 0.80))
        path.addLine(to: CGPoint(x: rect.width * 0.86, y: rect.height * 0.18))
        return path
    }
}

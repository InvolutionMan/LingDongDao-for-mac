...
收起状态有小任务在跑时，鼠标停在灵动岛的**左半边**会展开成那个 CLI 的实时详情（模型 / 思考程度 / 正在跑的工具 / 缓存命中 / token 用量），**右半边**则照常展开成完整的灵动岛主页（标签栏、Home 等一切照旧）。

- 左右判定按「指针 x 是否小于灵动岛中点」算，光标一进入岛内 0.3 秒（`minimumHoverDuration`）后判定，用的是那一刻的真实指针位置

...
设置 → LLM Providers 里多了一项 **DSH (dst)**（默认开），用量标签页会显示它的 Today / Week / 模型明细与费用。

数据来自 DSH 自己的账本 `~/.dsh/dsh-usage/usage-ledger.json`（按天 → 供应商 → 模型，自带 token 与 `cost`），所以**不需要解压任何 zstd 会话文件**（那些动辄几十 MB）。按最近 7 天聚合，模型按 token 量排序。

> 注意：账本按**天**分桶、没有会话概念，所以面板里的 "Session" 与 "Today" 数值相同；Today / Week 是精确的。
...
`pi / Codex / Claude / DSH 任务` · `媒体播放` · `计时器` · `提醒` · `录屏` · `下载` · `专注模式` · `摄像头/麦克风指示` · `音量等 sneak peek` · `文件架` · `大写锁定`

判定逻辑抽成纯函数 `islandHasSomethingToShow(_:)` 与 `islandShouldHide(hideOnClosed:autoHideWhenIdle:signals:)`（全屏覆盖菜单栏仍然优先隐藏），有 4 项单测覆盖「每个活动单独都能留住灵动岛」「安静时才隐藏」「全屏依然优先」。

> 注：在不带刘海的显示器上，Atoll 本来就会在空闲时收起灵动岛，所以这项设置在外接屏上看不出差别；它主要作用在自带刘海的屏幕（尤其是把收起高度调得比物理刘海更大时）。

### 计时器也有同样待遇

计时器在跑时和媒体一样，会缩到收起胶囊的右侧角落：一条 1pt 分割线 + 一个进度圆环（线宽 2.5、按 `timerColor` 着色，剩余比例用 `trim` 画出，超时后环保持满格），**圆环右边跟具体剩余时间**（12pt 等宽数字，和圆环同色，每秒刷新）。提示气泡里还有计时器名称。

- 媒体和计时器同时存在时**两个小圈都显示**，各带自己的分割线：`… 计时 │ ◯封面 │ ◯计时 12:34`
- 悬浮分区因此是三段（用徽章上报的分割线位置实时判定，没有徽章时仍按窗口中点分左右）：
  - 分割线左边 → 该 CLI 的实时详情
  - 封面那一段 → 播放主页（Home）
  - 计时圈那一段 → **计时器页**（`NotchTimerView`）
- 区间判定抽成纯函数 `islandHoverDestination(windowX:mediaDivider:timerDivider:windowWidth:)`，有单测覆盖两分割线/只有媒体/只有计时/都没有四种情况

### 悬浮：左半边看任务，右半边看主页

收起状态有小任务在跑时，鼠标停在哪里决定展开成什么：

- **媒体分割线左边**（任务那侧）→ 该 CLI 的实时详情（模型 / 思考程度 / 正在跑的工具 / 缓存命中 / token 用量）
- **媒体分割线右边**（专辑封面那一侧）→ 完整灵动岛主页，也就是和播放有关的那一屏
- 没有媒体在播（也就没有那条分割线）时，退化为按灵动岛中点分左右

分割线的位置由 `MusicCornerBadge` 通过 GeometryReader 实时上报到 coordinator（窗口坐标），悬浮判定用同一坐标系比较指针位置；光标进入岛内 0.3 秒（`minimumHoverDuration`）后判定那一刻的真实指针位置
- 悬浮打开后**在岛内滑动**也会跟着切换（coordinator 每 0.2 秒滴答一次、只在悬浮期间运行），不必先移出去再进来；切到媒体侧时会顺带回到 Home，也就是播放那一屏
- CLI 详情页现在**独占整个灵动岛**：标签栏等主页按钮不再叠在它上面（点击打开、调试自动打开也都一样），要离开详情只需把鼠标移开
- 没有 CLI 在跑时，左右半边行为一致，都是普通主页

### 媒体与任务同时进行

任务（pi / Codex / Claude / DSH）占着收起状态的灵动岛时，正在播放的媒体不会消失，也不会抢主位：它缩成右侧一个小圆圈，圆圈里只有专辑封面（像唱片一样匀速转动，**鼠标悬浮到灵动岛上就停下**，移开后从原角度继续），前面用一条 1pt 细线隔开——和手机灵动岛把通话放左边、音乐放右边一个思路。多个 CLI 堆叠时同样只在右侧放**一个**圆圈，纵向居中。
...
- 转速 8 秒一圈（20fps 驱动，够顺滑又不抢注意力）；封面绘制尺寸是圆圈的 1.45 倍，所以旋转时方形边角不会露出空隙；暂停时把旋转基准时间往后推，恢复时不会「跳」到本该转到的角度
...
1. **完全磁盘访问**（给 helper，不给 Atoll）：系统设置 → 隐私与安全性 → 完全磁盘访问 → 添加
   `~/Library/Application Support/Atoll/notify-bridge/AtollNotifyBridge.app/Contents/MacOS/atoll-notify-bridge`
2. **扩展授权**：第一条消息时灵动岛会弹一次提示，点允许
